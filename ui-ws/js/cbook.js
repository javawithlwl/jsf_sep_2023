...
function editContact(mobile){
    ...
    // 1. Fetch the contact by id

    fetch(`${baseUrl}/get-by/${mobile}`).then(res => res.json())
    .then(data => {
         
        // 2. Set the values to the form
        ...
        document.getElementById('idDob').valueAsDate = new Date(new Date(data.dob)+1);

        // 3. Change the button text to Update
